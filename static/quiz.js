function display_quiz_q(q){
    ...
    for (let option in q["options"]) {
        $form.append("<input type='radio' id='" + option + "name='" + q["id"]+ "' value='" + option + "'></input>")
        $form.append("<label for='" + option + "'>" + option + "</label><br></br>")
    }
    $form.append("<button type='button' id='submit-button' onclick='checkAnswers('" + q["id"] + "', '" + q["correct"] + "')'>Submit</button></form>")
...
function checkAnswers(id, correct) {
    ...
    let selectedAnswer = document.querySelector('input[name="' + id + '"]:checked').value;
...
function display_result_text(c){
    ...
    if (parseInt(c) < 2) {
        $group.append($("<div>").text("If you think you can do better, review the material and then try again!"))
    }
    else {
